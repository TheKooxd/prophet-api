  ...
  if(req.session.loggedIn && req.session.usr.role == "admin"){
    ...
    bcrypt.hash(req.query.pass, saltRounds, function(err, hash) {
      if(err) res.send("There was an error while hashing: " + err);
        ...
        collection.insert({name: req.query.name, role: req.query.role, pass: hash, changePass: true, eventRequirements: req.query.eventRequirements, loginId: req.query.loginId, verifiedEvents: JSON.stringify(new Array), events: JSON.stringify(new Array), reservedEvents: JSON.stringify(new Array)},function(e,docs){
          res.send("OK");
        })
      }
    });
    ...
    req.query.names.forEach(function(name, index){
      ...
      bcrypt.hash(pass, saltRounds, function(err, hash) {
        if(err) res.send("There was an error while hashing: " + err);
          ...
          collection.insert({name: name, role: req.query.role, groupId: req.query.groupName, pass: hash, changePass: true, eventRequirements: eventRequirements, loginId: name.replace(" ", "."), verifiedEvents: JSON.stringify(new Array), events: JSON.stringify(new Array), reservedEvents: JSON.stringify(new Array)},function(e,docs){
            profileCache.push({name: name, pass: pass, loginId: name.replace(" ", ".")});
            if(profileCache.length == req.query.names.length) res.send(JSON.stringify(profileCache))
          })
        }
      });
      })
...
router.get('/deleteParticipant', function(req, res) {
  if(req.session.loggedIn && req.session.usr.role == "admin" || req.session.usr._id == req.query.usrId && req.query != ""){
    var db = req.db;
            ...
            if(index + 1 == groupArray.length) {
              if(groupCache.length > 0 && req.session.usr.role == "admin") {
                if(!req.query.onlyMeta == "true") {
                  ...
                  if(response.length == 0) response = [{type: "group", id: groupCache[0].groupId, name: groupCache[0].groupId}]
                  else response.concat([{type: "group", id: groupCache[0].groupId, name: groupCache[0].groupId}])
                }
                data=true
              }
...
router.get('/setEvent', function(req, res) {
  if(req.session.loggedIn && req.session.usr.role == "admin"){
    var db = req.db;
    var collection = db.get('eventcollection');
    collection.insert({name: req.query.name, startTime: req.query.startTime, endTime: req.query.endTime, closes: req.query.closes, location: req.query.location, toInnostaja: req.query.toInnostaja, toEVI: req.query.toEVI, toIndividual: req.query.toIndividual, max: req.query.max, participants: JSON.stringify(new Array), reservedParticipants: JSON.stringify(new Array), info: req.query.info, releaseTime: req.query.releaseTime, jobs: req.query.jobs, type: req.query.type},function(e,docs){
        res.send("OK");
    });
  }
  else res.send(403)
